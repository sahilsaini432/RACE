...
class ECMGModel(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        retrieved_source_mask=None,
        retrieved_target_ids=None,
        retrieved_target_mask=None,
        use_cache=None,
        return_dict=None,
    ):
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        encoder_outputs_of_retrieved_target_ids = self.encoder(
            input_ids=retrieved_target_ids, attention_mask=retrieved_target_mask
        ).last_hidden_state
        ...
        combined_encoder_output = torch.cat(
            (
                self.W_c(encoder_outputs_of_input_source_ids) * (1 - sim),
                encoder_outputs_of_retrieved_target_ids * sim,
            ),
            dim=1,
        )
        ...
        if target_ids is not None:
            ...
            loss = loss_fct(
                shift_logits.view(-1, shift_logits.size(-1))[active_loss], shift_labels.view(-1)[active_loss]
            )
            ...
            preds = []
            device = source_ids.device  # Get device from input tensors
            zero = torch.tensor([0], dtype=torch.long, device=device)
            for i in range(source_ids.shape[0]):
                context = combined_encoder_output[i : i + 1]  # [1,seq_len,  dim ]
                context_mask = combined_encoder_mask[i : i + 1, :]
                beam = Beam(self.beam_size, self.sos_id, self.eos_id, device=device)  # Pass device to Beam
...
class Beam(object):
    def __init__(self, size, sos, eos, device="cpu"):  # Add device parameter
        self.size = size
        self.device = device
