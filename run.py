...
from util import (
    get_elapse_time,
    load_and_cache_commit_data,
    load_and_commit_data_with_retrieved_result,
    save_json_data,
)
...
def parse_args():
    ...
    parser.add_argument(
        "--ECMG_type",
        default="shared_encoders",
        choices=["shared_encoders"],
        type=str,
        required=False,
        help="the type of ECMG model",
    )
    parser.add_argument(
        "--base_model_type",
        default="codet5",
        ...
        type=str,
        required=False,
        help="the type of base model, like codet5, siamese network",
    )
    parser.add_argument(
        "--model_type",
        default="codet5",
        ...
        help="the type of pretrain model",
    )

    parser.add_argument(
        "--diff_type",
        type=str,
        default="contextual-medit",
        choices=[
            "plain_diff",
            "old-plain-diff",
            "medit",
            "old-medit",
            "contextual-palin_diff",
            "contextual-medit",
        ],
        help="plain_diff: only plain diff text; old-plain-diff: before and after three lines + plain diff \
                        ...
                        old-medit: old version + medit, oversion is the before and after three lines + old diff , \
    ...
    parser.add_argument(
        "--add_task_prefix", action="store_true", help="Whether to add task prefix for t5 and codet5"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="../saved_model/commit_msg_generation/java/tmp",
    ...
    parser.add_argument(
        "--train_filename",
        default="../data/commit_msg/java/contextual_medits/train.jsonl",
        type=str,
        help="The train filename. Should contain the .jsonl files for this task.",
    )
    parser.add_argument(
        "--dev_filename",
        default="../data/commit_msg/java/contextual_medits/valid.jsonl",
        type=str,
        help="The dev filename. Should contain the .jsonl files for this task.",
    )
    parser.add_argument(
        "--test_filename",
        default="../data/commit_msg/java/contextual_medits/test.jsonl",
        type=str,
        help="The test filename. Should contain the .jsonl files for this task.",
    )
    parser.add_argument(
        "--train_retireved_filename",
        default="../data/commit_msg/java/contextual_medits/train.jsonl",
        type=str,
        help="The train filename. Should contain the .jsonl files for this task.",
    )
    parser.add_argument(
        "--dev_retireved_filename",
        default="../data/commit_msg/java/contextual_medits/valid.jsonl",
        type=str,
        help="The dev filename. Should contain the .jsonl files for this task.",
    )
    parser.add_argument(
        "--test_retireved_filename",
        default="../data/commit_msg/java/contextual_medits/test.jsonl",
        type=str,
        help="The test filename. Should contain the .jsonl files for this task.",
    )
    ...
    parser.add_argument(
        ...
        default="../data/commit_msg/java/tmp/codet5_retrieval_result",
        type=str,
        help="The test filename. Should contain the .jsonl files for this task.",
    )
    parser.add_argument(
...
def set_dist(args):
    ...
    if args.local_rank == -1 or args.no_cuda:
        # Check for MPS (Apple Silicon) support first, then CUDA, then fallback to CPU
        if torch.backends.mps.is_available() and not args.no_cuda:
            device = torch.device("mps")
            args.n_gpu = 1  # MPS is treated as single GPU
        ...
        # Setup for distributed data parallel (CUDA only)
        torch.cuda.set_device(args.local_rank)
        device = torch.device("cuda", args.local_rank)
        torch.distributed.init_process_group(backend="nccl")
        args.n_gpu = 1

    print(f"Device: {device}, GPU count: {args.n_gpu}")
...
def main(args):
    ...
    if args.do_train:
        ...
        train_examples, train_data = load_and_cache_commit_data(
            args, args.train_filename, pool, tokenizer, "train", is_sample=args.debug
        )
        ...
        optimizer = torch.optim.AdamW(
            params=optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon
        )
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.do_eval:
                eval_examples, eval_data = load_and_cache_commit_data(
                    args, args.dev_filename, pool, tokenizer, "dev", only_src=True, is_sample=args.debug
                )
    ...
    if args.do_test:
        ...
        if args.load_model_path is not None:
            ...
            model.load_state_dict(torch.load(args.load_model_path))
        eval_examples, eval_data = load_and_cache_commit_data(
            args, args.test_filename, pool, tokenizer, "test", only_src=True, is_sample=args.debug
        )

        pred_nls = eval_bleu_epoch(
            args, eval_data, eval_examples, model, tokenizer, "test", beam_size=args.beam_size
        )
    ...
    if args.do_retrieval:
        ...
        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=args.eval_batch_size, num_workers=4, pin_memory=True
        )

        eval_examples, eval_data = load_and_cache_commit_data(
            args, args.retrieval_filename, pool, tokenizer, "train", only_src=True, is_sample=args.debug
        )
        eval_sampler = SequentialSampler(eval_data)
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
        ...
        model = model.module.encoder if hasattr(model, "module") else model.encoder
        ...
        for batch in train_dataloader:
            with torch.no_grad():
                ...
                source_mask = source_ids.ne(tokenizer.pad_token_id)
                # [bs, hid_dim]
                train_code_vec = model(
                    input_ids=source_ids, attention_mask=source_mask
                ).last_hidden_state  # [bs, sequence_length, dim]
        ...
        sort_ids = np.argsort(scores, axis=-1, kind="quicksort", order=None)[
            :, ::-1
        ]  # [num_of_eval_samples,num_of_train_samples]
        ...
        for idx in rank1_result:
            ...
            retrieval_results.append(
                {"diff": retrieval_result.source.split(), "msg_token": retrieval_result.target.split()}
            )
            pred_nls.append(retrieval_result.target)

        predictions = []
        output_fn = os.path.join(
            args.retrieval_result_dir, "%s.retireval.output" % args.retrieval_result_filename
        )
...
def ECMG(args):
    ...
    if args.do_train:
        ...
        # Prepare training data loader
        train_input_examples, train_retrieved_examples, train_data = (
            load_and_commit_data_with_retrieved_result(
                args,
                args.train_filename,
                args.train_retireved_filename,
                pool,
                tokenizer,
                "train",
                is_sample=args.debug,
            )
        )
        train_sampler = RandomSampler(train_data) if args.local_rank == -1 else DistributedSampler(train_data)
        train_dataloader = DataLoader(
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            ...
            if args.do_eval:
                ...
                eval_examples, eval_retrieved_examples, eval_data = (
                    load_and_commit_data_with_retrieved_result(
                        args,
                        args.dev_filename,
                        args.dev_retireved_filename,
                        pool,
                        tokenizer,
                        "dev",
                        only_src=True,
    ...
    if args.do_test:
        ...
        #                                                     only_src=True, is_sample=args.debug)
        eval_examples, eval_retrieved_examples, eval_data = load_and_commit_data_with_retrieved_result(
            args,
            args.test_filename,
            args.test_retireved_filename,
            pool,
            tokenizer,
            "test",
            only_src=True,
