...
def convert_clone_examples_to_features(item):
    ...
    code2 = tokenizer.encode(
        target_str, max_length=args.max_source_length, padding="max_length", truncation=True
    )
...
def load_and_commit_data_with_retrieved_result(
    args, input_filename, retireved_filename, pool, tokenizer, split_tag, only_src=False, is_sample=False
):
    # cache the data into args.cache_path except it is sampled
    # only_src: control whether to return only source ids for bleu evaluating (dev/test)
    # return: examples (Example object), data (TensorDataset)
    ...
    tuple_input_examples = [
        (example, idx, tokenizer, args, split_tag) for idx, example in enumerate(input_examples)
    ]
    tuple_retrieved_examples = [
        (example, idx, tokenizer, args, "train") for idx, example in enumerate(retrieved_examples)
    ]

    input_features = pool.map(
        convert_examples_to_features, tqdm(tuple_input_examples, total=len(tuple_input_examples))
    )
    retrieved_features = pool.map(
        convert_examples_to_features, tqdm(tuple_retrieved_examples, total=len(tuple_retrieved_examples))
    )
    ...
    if split_tag == "test" or only_src:
        ...
        data = TensorDataset(
            all_input_source_ids, all_target_ids, all_retrieved_source_ids, all_retrieved_target_ids
        )
        # if args.local_rank in [-1, 0] and not is_sample:
        #     torch.save(data, cache_fn)
    if args.debug:
...
def load_and_cache_clone_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    cache_fn = "{}/{}.pt".format(
        args.cache_path, split_tag + "_all" if args.data_num == -1 else "_%d" % args.data_num
    )
    examples = read_examples(filename, args.data_num, args.task)
    if is_sample:
    ...
    if os.path.exists(cache_fn):
        ...
        features = pool.map(
            convert_clone_examples_to_features, tqdm(tuple_examples, total=len(tuple_examples))
        )
        all_source_ids = torch.tensor([f.source_ids for f in features], dtype=torch.long)
...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if os.path.exists(cache_fn):
        ...
        tuple_examples = [(example, idx, tokenizer, args) for idx, example in enumerate(examples)]
        features = pool.map(
            convert_defect_examples_to_features, tqdm(tuple_examples, total=len(tuple_examples))
        )
