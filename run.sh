...
test_file=dataset/next.js/model_data/test.jsonl
...
output_dir=saved_model/codet5/${repo}/
mkdir -p $output_dir
echo $output_dir
echo "============TRAINING============"
 CUDA_VISIBLE_DEVICES=0 python3 run.py  --do_train --do_eval   --do_test --eval_frequency 100 \
  --run_codet5 \
...
load_model_path=saved_model/codet5/${repo}/checkpoint-best-bleu/pytorch_model.bin
 CUDA_VISIBLE_DEVICES=0  python3 run.py   --do_retrieval \
 --run_codet5 \
 --is_cosine_space \
 --train_filename ${train_file} \
 --cache_path $cache_dir \
